...
class CobolParser::Scanner < Lex::Lexer
  option :caseless

  # cb_reset_in_procedure => in_procedure = false, cb_set_in_procedure => in_procedure = true
  attr_accessor :in_procedure
  # cb_reset_78 => lev78.clear, cb_add_78 => lev78.unshift(field)
  attr_reader :lev78
  ...
  before_scan {
    if @cb.current_program
      if @cb.current_program.decimal_point == "."
  ...
  rule(/[0-9]+/) {
    ...
    if @integer_is_label
      # integer label
      yylval.value = @cb.build_reference(yytext)
  ...
  rule(/SWITCH[ ]+1/) {
    yylval.value = @cb.build_reference("SWITCH-1")
    set_location(yylval.value)
    :WORD
  }

  rule(/SWITCH[ ]+2/) {
    yylval.value = @cb.build_reference("SWITCH-2")
    set_location(yylval.value)
    :WORD
  }

  rule(/SWITCH[ ]+3/) {
    yylval.value = @cb.build_reference("SWITCH-3")
    set_location(yylval.value)
    :WORD
  }

  rule(/SWITCH[ ]+4/) {
    yylval.value = @cb.build_reference("SWITCH-4")
    set_location(yylval.value)
    :WORD
  }

  rule(/SWITCH[ ]+5/) {
    yylval.value = @cb.build_reference("SWITCH-5")
    set_location(yylval.value)
    :WORD
  }

  rule(/SWITCH[ ]+6/) {
    yylval.value = @cb.build_reference("SWITCH-6")
    set_location(yylval.value)
    :WORD
  }

  rule(/SWITCH[ ]+7/) {
    yylval.value = @cb.build_reference("SWITCH-7")
    set_location(yylval.value)
    :WORD
  }

  rule(/SWITCH[ ]+8/) {
    yylval.value = @cb.build_reference("SWITCH-8")
    set_location(yylval.value)
    :WORD
  }
  ...
  rule(/[A-Z0-9]([_A-Z0-9-]*[A-Z0-9]+)?/) {
    # Check word length
    @cb.warning("User defined name must be less than 32 characters:%s", yytext) if yytext.length > 31

    # Check FUNCTION name without keyword
    if @in_procedure && @cb.functions_are_all
      cbp = @cb.lookup_intrinsic(yytext, true)
      if cbp
        yylval.value = @cb.build_reference(yytext)
        set_location(yylval.value)
    ...
    # Check reserved word
    token = @cb.lookup_reserved_word(yytext)
    if token
      yylval.value = nil
      next token
    end
    ...
    @lev78.each do |p78|
      if yytext.casecmp?(p78.name)
        if @cb.non_const_word
          @cb.error("CONSTANT (78 level) may not be used here - '%s'", yytext)
    ...
    if @cb_force_pid_literal
      ...
      @cb_force_pid_literal = false
      yylval.value = @cb.build_alphanumeric_literal(yytext)
      set_location(yylval.value)
      next :PROGRAM_NAME
    end
  ...
  def set_location(tree) # rubocop:disable Naming/AccessorMethodName
    tree.source_file = @cb.source_file
    tree.source_line = @cb.source_line
  end
  ...
  def read_literal(mark)
    ...
    if buff.empty?
      @cb.warning("Alphanumeric literal has zero length")
      @cb.warning("A SPACE will be assumed")
      buff = " "
    end

    yylval.value = @cb.build_alphanumeric_literal(buff)
    set_location(yylval.value)
    :LITERAL
  end
  ...
  def scan_x(text)
    ...
    if md
      yylval.value = @cb.build_alphanumeric_literal([md[1]].pack("H*"))
      set_location(yylval.value)
    else
      @cb.error("Invalid X literal: %s", text)
  ...
  def scan_h(text)
    ...
    if md
      yylval.value = @cb.build_numeric_literal(:UNSIGNED, md[1].to_i(16).to_s, 0)
      set_location(yylval.value)
    else
      @cb.error("Invalid H literal: %s", text)
      yylval.value = @cb.constants.error_node
    end
    :LITERAL
  end
  ...
  def scan_numeric(text)
    ...
    @cb.error("Invalid numeric literal") if text.include?(".")
    @cb.error("Invalid numeric literal") if text.include?(",")

    yylval.value = @cb.build_numeric_literal(sign, text, scale)
    set_location(yylval.value)
    :LITERAL;
  end
  ...
  def scan_picture(text)
    ...
    yylval.value = @cb.build_picture(text)
    :PICTURE
  end

  def count_lines(text)
    @cb.source_line += text.count("\n")
  end
