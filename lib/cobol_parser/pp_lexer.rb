...
class CobolParser::PPLexer < Lex::Lexer
  include CobolParser::MbsMixin
  ...
  rule(:*, /^\*.*/,
       :*, /^\/.*/) {
    ppecho(" ")
    ppecho(yytext) if !@cb.fixed_source_format?
  }
  ...
  rule_for(:PROCESS_STATE) do
    ...
    rule(/.*/) { @cb.warning("PROCESS statement is ignored") }
  end
  ...
  rule("AUTHOR",
    ...
    if @cb.verify(@cb.author_paragraph, yytext)
  ...
  rule(/EJECT\.?/,
    ...
    if @cb.verify(@cb.eject_statement, yytext)
      # do nothing for now
    else
      echo
    end
  ...
  def open(name, replace_list = nil)
    ...
    @cb.depend_list << name if @cb.depend_file
  ...
  def copy(name, lib = nil, replace_list = nil)
    ...
    @cb.error("%s: %s", name, "No such file")
  ...
  def check_directive(buff)
    ...
    if sbuff.length < 2 || sbuff[0] != "SOURCE"
      @cb.warning("Invalid directive - ignored")
      return buff
    end
    ...
    when 2
      if sbuff[1].casecmp?("FIXED")
        @cb.source_format = Context::FORMAT[:FIXED]
      elsif sbuff[1].casecmp?("FREE")
        @cb.source_format = Context::FORMAT[:FREE]
      end
      return buff
    when 3
  ...
  def ppinput
    ...
    while (buff = ppin.gets)
      ...
      buff = NKF.nkf("-w", buff)
      buff = " " + buff if !@cb.fixed_source_format? && buff[0] != " " && buff[0] != "\n"
      buff.gsub!(/\r\n/, "\n")
      if buff.index("\t")
        n = 0
        buff = buff.chars.map { |c|
          ...
          if c == "\t"
            num_spaces = 1
            num_spaces += (@cb.tab_width - (n % @cb.tab_width)) % @cb.tab_width
            " " * num_spaces
          else
            c
          end
      ...
      return buff if !@cb.fixed_source_format?
      ...
      if @cb.flag_mfcomment && (buff[0] == "*" || buff[0] == "/")
        @newline_count += 1
        next
      end
      ...
      when "*", "/"
        ...
        # invalid indicator
        @cb.error("Invalid indicator '%c' at column 7", buff[6])
        return nil
      end
      ...
      if mbs_length(buff) > @cb.text_column + 1
        # show warning if it is not whitespaces
        if @cb.warn_column_overflow && @last_line_2 < @cb.source_line - 1
          if /[^ \n]/ =~ mbs_slice_range(buff, @cb.text_column..-1)
            @cb.warning("Source text after column %d", @cb.text_column)
          end
          # remove it
          buff = mbs_slice_nth_length(buff, 0, @cb.text_column)
        end
      end
      ...
      if continuation
        ...
        if @consecutive_quotation
          if bp[0] == @quotation_mark && bp[1] == @quotation_mark
            ...
            @cb.error("Invalid line continuation")
            return nil
          end
          @quotation_mark = nil
          @consecutive_quotation = false
      ...
      if @quotation_mark
        buff += " " * (@cb.text_column - mbs_length(buff))
      else
        buff.chomp!
        buff.gsub!(/ +$/, "")
