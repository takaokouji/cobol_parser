...
module CobolParser
  ...
  class Tree::Field < Tree
    ...
    Key = Struct.new(
      ...
      :key, # KEY
      :ref, # reference used in SEARCH ALL
      :val, # value to be compared in SEARCH ALL
      keyword_init: true
    )
    ...
    module Helper
      ...
      def cb_build_field_tree(level, name, last_field, storage, fn)
        return cb_error_node if level == cb_error_node || name == cb_error_node

        # check the level number
        lv = level.level_from_name
        return cb_error_node if lv == 0

        # build the field
        f = cb_build_field(name)
        f.storage = storage
        ...
        if last_field && (last_field.level == 77 && f.level != 1 && f.level != 77 && f.level != 66 && f.level != 88)
          cb_error_x(name, "Level number must begin with 01 or 77")
          return cb_error_node
        end

        # checks for redefinition
        r = name
        if cb_warn_redefinition && r.word.count > 1
        ...
        if f.level == 1 || f.level == 77
          ...
          last_field.each_parent do |p|
            ...
            break if cb_relax_level_hierarchy && p.level < f.level
          end
          if !same_level_flag
            if cb_relax_level_hierarchy
              dummy_fill = cb_build_filler
              field_fill = cb_build_field(dummy_fill)
              cb_warning_x(name, "No previous data item of level %02d", f.level)
              ...
              cb_error_x(name, "No previous data item of level %02d", f.level)
              return cb_error_node
            end
          end
        end
      ...
      def cb_validate_field(p)
        p.validate
      end

      attr_accessor :last_real_field

      def cb_clear_real_field
        @last_real_field = nil
      end
    end
    ...
    def validate_field_1
      ...
      if flag_occurs
        ...
        index_list&.each_chain do |l|
          cb_field(l.value).flag_is_global = flag_is_global
        end
      end
      ...
      if children
        ...
        if pic.nil? && need_picture
          if storage == :SCREEN # rubocop:disable Style/GuardClause
            # TODO: field.c:513:
            raise NotImplementedError
          elsif flag_item_78 && values && values.value != cb_error_node # rubocop:disable Lint/DuplicateBranch
            # TODO: field.c:524
            raise NotImplementedError
          else
            if flag_item_78
              cb_error_x(x, "Value required for constant item '%s'", name)
            else
              cb_error_x(x, "PICTURE clause required for '%s'", name)
            end
            return false
          end
        end

        cb_error_x(x, "'%s' cannot have PICTURE clause", name) if !pic.nil? && !need_picture

        # validate USAGE
        case usage
        when :SIGNED_CHAR
          @usage = :COMP_5
          @pic = cb_build_picture("S99")
          @flag_real_binary = true
        when :SIGNED_SHORT
          @usage = :COMP_5
          @pic = cb_build_picture("S9(4)")
          @flag_real_binary = true
        when :SIGNED_INT
          @usage = :COMP_5
          @pic = cb_build_picture("S9(9)")
          @flag_real_binary = true
        when :SIGNED_LONG
          @usage = :COMP_5
          @pic = cb_build_picture("S9(18)")
          @flag_real_binary = true
        when :UNSIGNED_CHAR
          @usage = :COMP_5
          @pic = cb_build_picture("99")
          @flag_real_binary = true
        when :UNSIGNED_SHORT
          @usage = :COMP_5
          @pic = cb_build_picture("9(4)")
          @flag_real_binary = true
        when :UNSIGNED_INT
          @usage = :COMP_5
          @pic = cb_build_picture("9(9)")
          @flag_real_binary = true
        when :UNSIGNED_LONG
          @usage = :COMP_5
          @pic = cb_build_picture("9(18)")
          @flag_real_binary = true
        when :BINARY, :PACKED
          cb_error_x(x, "'%s' PICTURE clause not compatible with USAGE", name) if pic.category != :NUMERIC
        when :COMP_5, :COMP_X
          if pic && pic.category != :NUMERIC && pic.category != :ALPHANUMERIC
            cb_error_x(x, "'%s' PICTURE clause not compatible with USAGE", name)
          end
        end
        ...
        if flag_justified && !%i[ALPHABETIC ALPHANUMERIC].include?(pic.category)
          cb_error_x(x, "'%s' cannot have JUSTIFIED RIGHT", name)
        end
    ...
    def resolve_redefines(redefines)
      ...
      # check qualification
      if r.chain
        cb_error_x(x, "'%s' cannot be qualified here", name)
        return
      end

      # check subscripts
      if r.subs
        cb_error_x(x, "'%s' cannot be subscripted here", name)
        return
      end
      ...
      if parent&.children
        ...
        if !f
          cb_error_x(x, "'%s' undefined in '%s'", name, parent.name)
          return
        end
      else
        return if redefines.ref == cb_error_node
      ...
      # check level number
      if f.level != level
        cb_error_x(x, "Level number of REDEFINES entries must be identical")
        return
      end
      if f.level == 66 || f.level == 88
        cb_error_x(x, "Level number of REDEFINES entry cannot be 66 or 88")
        return
      end

      if !cb_indirect_redefines && f.redefines
        cb_error_x(x, "'%s' not the original definition", f.name)
        return
      end
    ...
    def setup_parameters
      ...
      if children
        ...
        case usage
        when :BINARY
          @flag_binary_swap = true if cb_binary_byteorder == :BIG_ENDIAN
        when :INDEX
          @pic = cb_build_picture("S9(9)")
        when :LENGTH
          @pic = cb_build_picture("9(9)")
        when :POINTER, :PROGRAM_POINTER
          @pic = cb_build_picture("9(10)")
        when :FLOAT
          @pic = cb_build_picture("S9(7)V9(7)")
        when :DOUBLE
          @pic = cb_build_picture("S9(9)V9(9)")
        when :COMP_5, :COMP_X
          if pic.category == :ALPHANUMERIC
            ...
            @pic = cb_build_picture(s)
          end
          @flag_binary_swap = true if usage == :COMP_X && cb_binary_byteorder == :BIG_ENDIAN
        end
      end
    end
        ...
        if c.redefines
          ...
          if c.level != 66 && c.size * c.occurs_max > c.redefines.size * c.redefines.occurs_max
            if cb_larger_redefines_ok
              cb_warning_x(c, "Size of '%s' larger than size of '%s'", c.name, c.redefines.name)
              result += (c.size * c.occurs_max) - (c.redefines.size * c.redefines.occurs_max)
            else
              cb_error_x(c, "Size of '%s' larger than size of '%s'", c.name, c.redefines.name)
            end
          end
        else
          ...
          # word alignment
          if flag_synchronized && cb_verify(cb_synchronized_clause, "SYNC")
            align_size = 1
            case c.usage
    ...
    def compute_size_elementary_item
      ...
      when :BINARY, :COMP_5
        ...
        if pic_size > 18
          ...
          cb_error_x(self, "'%s' binary field cannot be larger than 18 digits", name)
        end
    ...
    def compute_size
      ...
      if redefines&.flag_external &&
        ...
        if cb_larger_redefines_ok
          ...
          cb_error_x(self, "Size of '%s' larger than size of '%s'", name, redefines.name)
        end
      end
    ...
    def validate_field_value
      # TODO: cb_validate_move
      cb_validate_move(values.value, self, true) if values
